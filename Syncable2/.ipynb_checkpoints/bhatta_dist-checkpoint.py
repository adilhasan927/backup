...
def bhatta_dist(X1, X2, method='integrate', N_BINS=128, **kwargs):
    ...
    if method == 'noiseless':
        ...
        ###Bin the values into a specified number of bins (This is sensitive to N_BINS)
        #N_BINS = 128
        #Bin the values:
        ...
        for x in xs:
            ...
            a += (p1*(max(cX)-min(cX))/N_STEPS)
            
    elif method == 'integrate':
        d1 = get_density(X1)
        d2 = get_density(X2)
        bht = quad(lambda x: sqrt(d1(x)*d2(x)), min(cX), max(cX), full_output=0)
        #print(bht)
        bht=bht[0]
    else:
